...
class FileFieldType extends FieldType
{
    ...
    public function getViewFunctionContents()
    {
            ...
            if ($' . $slugID . ' > 0) {
                ...
                if ($fp->canViewFile()) {
                    ...
                    if ($c instanceof Page) {
                        ...
                        $urls[\'download\'] = URL::to(\'/download_file\', $' . $slugID . ', $cID);
                    }
                    $' . $slugFile . '->urls = $urls;
                    $' . $slug . ' = $' . $slugFile . ';
                }
        ...
        if ($' . $this->data['slug'] . '_id > 0) {
	        ...
	        if ($fp->canViewFile()) {
	            $urls = array(\'relative\' => $' . $this->data['slug'] . '_file->getRelativePath());
		        $c = Page::getCurrentPage();
		        if ($c instanceof Page) {
			        $cID = $c->getCollectionID();
			        $urls[\'download\'] = URL::to(\'/download_file\', $' . $this->data['slug'] . '_id, $cID);
		        }
