    ...
    public static function run($postData = array())
    {
        ...
        if (!empty(self::$errors)) {
                        ...
                        switch ($assetEntriesType) {
                            ...
                            case 'register':
                                if (!isset($assetsLists[$assetFunctionName])) {
                                    $controllerFunctions[$assetFunctionName]['lines'][] = '$al = \Concrete\Core\Asset\AssetList::getInstance();';
            ...
            if (count($tabs) > 1) {
                ...
                foreach ($tabs as $tab) {
                    $tabsArray[] = self::getTabs(1) . 'array(\'' . $tab[0] . '-\' . $unique_tabs_id, t(\'' . $tab[1] . '\')' . (isset($tab[2]) && $tab[2] === true ? ', true' : null) . ')';
                }
                $formFields[] = '<?php  $unique_tabs_id = Core::make(\'helper/validation/identifier\')->getString(18);
$tabs = array(' . PHP_EOL . implode(',' . PHP_EOL, $tabsArray) . PHP_EOL . ');
echo Core::make(\'helper/concrete/ui\')->tabs($tabs); ?>';
                if (!empty($tabsFields)) {
                    foreach ($tabsFields as $k => $v) {
                        $formFields[] = '<div class="ccm-tab-content" id="ccm-tab-content-' . $k . '-<?php  echo $unique_tabs_id; ?>">
    ' . implode('', $v) . '
</div>';
                    }
                }
