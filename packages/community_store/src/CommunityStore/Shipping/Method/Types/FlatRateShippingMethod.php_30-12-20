...
//        $this->set("states", $statelist);
        $this->set("countryList", Core::make('helper/lists/states_provinces')->getStateProvinceArray('AE'));
    ...
    public function isWithinSelectedCountries()
    {
        ...
        $custCountry = $customer->getValue('shipping_address')->state_province;
    ...
    public function getQuantityBasedRate($shippableItems)
    {
        ...
        foreach ($shippableItems as $item) {
            ...
            if($product->getAttribute('heavy_shipping')){
                $heavy_shipping = 1;
            }
        }
//        if ($totalQty > 1) {
...
//        
        ...
        if($heavy_shipping){
            $shippingTotal = $peritemrate ? $peritemrate : $baserate;
        }else{
            $shippingTotal = $baserate;
        }
