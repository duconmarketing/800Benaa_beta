...
class ProductList extends AttributedItemList
{
    ...
    public function finalizeQuery(\Doctrine\DBAL\Query\QueryBuilder $query)
    {
        ...
        if ($this->search) {
            //--------------- Added on 10/6/2020 to improve search - by Sabin ------------------------
            $keyArray = explode(' ', $this->search);   
            foreach ($keyArray as $row){
                $query->andWhere('pName like ?')->setParameter($paramcount++, '%'. $row. '%');
            }
            $query->orWhere('pSKU like ?')->setParameter($paramcount++, '%'. $this->search. '%');
            $query->orderBy('CASE WHEN pName LIKE "'.$this->search.'%" THEN 1 WHEN position("'. $this->search .'" IN pName) = position(" " IN pName)+1 THEN 2 ELSE 3 END');
            //-------------- Changes end here - OLD code commented below -----------------------------
            
//            $query->andWhere('pName like ?')->setParameter($paramcount++, '%'. str_replace(' ', '%', $this->search). '%')->orWhere('pSKU like ?')->setParameter($paramcount++, '%'. $this->search. '%');
        }
